...
#----------------------------------------------------------
def generate_reg_rst(
    nbits : int,
    version: str, 
    target_folder=".",
    template="Reg-rst-template.ice"):
    """Generate one register with reset"""

    #-- Nombre del fichero destino
    file_name = f"{nbits:02}-Reg-rst.ice"
    ...
    new_ice = ice_orig.replace(f"Reg-rst-02-verilog", f"{block_name}")

    #-- Version del componente. Version plantilla: 0.4
    new_ice = new_ice.replace("0.2", f"{version}")
...
#-- Main
TARGET = "../blocks/Regs-rst"
VERSION = "0.5"
#generate_reg_rst(3, VERSION, TARGET)
for i in range(2,33):
